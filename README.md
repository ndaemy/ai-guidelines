...
- OpenCode / Claude Code 공용 [AGENTS.md](./shared/AGENTS.md)
  - Commit Rules
  - Branch Strategy
  - Pull Request Rules
...
도구에 종속되지 않는 공용 가이드라인은 `shared/`에 두고, 각 도구가 기대하는 파일명/경로로 symlink를 겁니다.
Claude Code는 `AGENTS.md`를 네이티브로 읽지 않기 때문에 글로벌 메모리 경로인 `~/.claude/CLAUDE.md`로 링크합니다.
